...
function NotifyForm() {
  ...
  if (submitted) {
    return <p className="summit__manifest">You're on the manifest. We'll signal you from the summit.</p>
  }
  return (
    <form className="summit__form" onSubmit={onSubmit}>
      <input
        type="email"
        required
...
export default function Overlays({ progress: p }: { progress: MotionValue<number> }) {
  ...
  const formOp = useTransform(p, [0.9, 0.914], [0, 1])
  ...
  return (
    <>
        ...
        <div className="checkpoint">
          ...
          <div className="checkpoint__links">
            <a
              ...
              href="https://forms.cloud.microsoft/Pages/DesignPageV2.aspx?subpage=design&FormId=jgbSOiU_hk2ePRxkXvnrgKYHdc4cd-FPvrQfigzveYBUQVhXTkUwU0tMTkkzQ1VKNUNYQzFMS1M4Qi4u&Token=9db8720df5ee41a29ca25a10fd400bf0"
              target="_blank"
      ...
      <Layer progress={p} band={[0.655, 0.672, 0.685, 0.702]} align="right">
        <div className="checkpoint">
          <p className="tag">CHECKPOINT D · 4 450 M</p>
          <h3 className="checkpoint__title">Registrations</h3>
          <p className="checkpoint__desc">The route opens to all schools soon.</p>
          <span className="stamp stamp--accent">OPENING EARLY</span>
        </div>
      </Layer>
      ...
      >
        <div className="summit">
          ...
          <motion.div style={{ opacity: formOp }}>
            <p className="summit__cta">Be the first to know when registrations open.</p>
            <NotifyForm />
          </motion.div>
          ...
          <motion.p className="summit__foot" style={{ opacity: footOp }}>
            In aid of the Sri Jayadeva Institute of Cardiovascular Sciences · 13 &amp; 14 August
            2026
          </motion.p>
          <motion.p className="summit__foot" style={{ opacity: footOp }}>
            Contact&nbsp;
